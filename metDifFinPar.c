...
#pragma omp parallel private(i,j,tid,ini,ult,andamento,trabalho) shared(p,nt, chunk,resto)
{
	ini=p;
	ult=p;
	...
	for(i=0;i<trabalho;i++)			
	{
		for(j=0;j<d->nCol;j++)
		{
			if(ult->cc==0)
			{
				//Laplace Discretizado
...
double att(struct quad *p,struct data *d)//Operador Diferencial
{
	struct quad *ini,*ult;
	int i,j,tid;
	double erro=0;
...
#pragma omp parallel private(i,j,tid,ini,ult,andamento,trabalho) shared(p,nt, chunk,resto) reduction(max:erro)
{
	...
	for(i=0;i<trabalho;i++)			
	{
		for(j=0;j<d->nCol;j++)
		{
			...
			{
		if(ult->a!=ult->d){
			if(erro<fabs((ult)->d-(ult)->a)){
				erro=fabs((ult)->d-(ult)->a);
			}
		ult->a=ult->d;
		} 
			}
...
int main(int argc, char *argv[])
{
	...
	do{
		...
		data->erro=att(map,data);//Atualiza Matrizes
	}while(tol<data->erro);
